...
export default async function StockDetailPage({ params }: StockPageProps) {
  ...
  const priceActionAnalysis = isIntradayLatest ? createTechnicalSnapshot(candles, supabaseScore).analysis : analysis;
  const change = latest.close - previous.close;
  const changePercent = round((change / previous.close) * 100);
  const isUp = change >= 0;

  return (
    <main className="min-h-screen bg-slate-50 pb-10 text-slate-950 dark:bg-[#071126] dark:text-slate-100">
      <div className="sticky top-0 z-50 border-b border-sky-200 bg-white/88 shadow-[0_10px_32px_rgba(15,23,42,0.08)] backdrop-blur-xl transition-colors dark:border-cyan-300/10 dark:bg-[#071126]/88 dark:shadow-[0_10px_32px_rgba(0,0,0,0.22)]">
        <div className="mx-auto flex min-h-14 w-full max-w-7xl items-center justify-between gap-2 px-2 sm:gap-3 sm:px-6 lg:px-8">
          ...
          <div className="min-w-0 flex-1 px-1 text-center">
            <div className="flex min-w-0 items-center justify-center gap-2">
              <p className="truncate text-base font-black tracking-normal text-slate-950 dark:text-white sm:text-lg">{stock.symbol}</p>
              <span className="rounded-lg border border-cyan-300 bg-cyan-50 px-2 py-0.5 text-[11px] font-semibold text-cyan-700 dark:border-cyan-300/20 dark:bg-white/5 dark:text-cyan-100">
                {stock.exchange}
              </span>
            </div>
            <p className="mx-auto mt-0.5 max-w-[150px] truncate text-xs text-slate-500 dark:text-slate-400 min-[390px]:max-w-[190px] sm:hidden">{stock.name}</p>
          </div>
          <ThemeToggle />
        </div>
      ...
      <section className="border-b border-sky-200 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.16),transparent_34%),linear-gradient(180deg,#f8fafc,#eef6fb)] dark:border-cyan-300/10 dark:bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.14),transparent_34%),linear-gradient(180deg,#08152d,#071126)]">
        <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-6 sm:py-6 lg:px-8">
          <div className="grid gap-4 rounded-2xl border border-sky-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/[0.03] dark:border-cyan-300/10 dark:bg-white/[0.03] dark:shadow-[0_20px_60px_rgba(0,0,0,0.18)] dark:ring-white/5 lg:grid-cols-[1fr_auto] lg:items-center lg:p-5">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-4xl font-black tracking-normal text-slate-950 dark:text-white sm:text-5xl">
                  {stock.symbol}
                </h1>
                <span className="rounded-xl border border-cyan-300 bg-cyan-50 px-2.5 py-1 text-xs font-bold text-cyan-700 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-100">
                  {stock.exchange}
                </span>
              </div>
              <p className="mt-2 line-clamp-1 text-sm text-slate-700 dark:text-slate-300 sm:text-base">{stock.name}</p>
              <p className="mt-1 text-xs font-semibold uppercase tracking-normal text-slate-500">
                {stock.sector}
              </p>
            </div>

            <div className="flex items-center gap-4 rounded-2xl border border-sky-200 bg-slate-50 p-4 dark:border-cyan-300/10 dark:bg-[#030816]/70">
      ...
      <StockDetailTabs
        ...
        <div className="space-y-5">
          <PriceActionSection analysis={priceActionAnalysis.priceAction} />

          <WyckoffLiteSection analysis={priceActionAnalysis.priceBehavior} />
        </div>
        }
        indicators={
        <div className="space-y-5 lg:grid lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start lg:gap-5 lg:space-y-0">
          ...
          <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/[0.03] dark:border-cyan-300/10 dark:bg-[#09152c]/95 dark:shadow-[0_20px_60px_rgba(0,0,0,0.18)] dark:ring-white/5">
...
function QuickSignalsSection({
  ...
  return (
    <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/[0.03] dark:border-cyan-300/10 dark:bg-[#09152c]/95 dark:shadow-[0_20px_60px_rgba(0,0,0,0.18)] dark:ring-white/5">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {signalGroups.map((group) => (
          <div key={group.category} className="rounded-2xl border border-sky-100 bg-slate-50 p-4 dark:border-cyan-300/10 dark:bg-[#030816]/70">
            <h3 className="text-sm font-semibold text-slate-950 dark:text-white">
              {categoryLabelsVi[group.category]}
            </h3>
            <div className="mt-3 space-y-2">
...
function PriceActionSection({ analysis }: { analysis: PriceActionCore }) {
  const structureBias = getMarketStructureDisplayBias(analysis.marketStructure);
  ...
  return (
    <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/[0.03] dark:border-cyan-300/10 dark:bg-[#09152c]/95 dark:shadow-[0_20px_60px_rgba(0,0,0,0.18)] dark:ring-white/5">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        <article className="rounded-2xl border border-sky-100 bg-slate-50 p-4 dark:border-cyan-300/10 dark:bg-[#030816]/70">
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-sm font-semibold text-slate-950 dark:text-white">Cấu trúc giá</h3>
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${getBehaviorSentimentClass(structureBias)}`}>
              {getBehaviorSentimentLabel(structureBias)}
            </span>
          </div>
...
function getMarketStructureDisplayBias(
  marketStructure: PriceActionCore["marketStructure"],
): Signal["sentiment"] {
  if (marketStructure.lastBreakType === "breakout") return "bullish";
  if (marketStructure.lastBreakType === "breakdown") return "bearish";
  return marketStructure.shortTermBias;
}
  ...
  return (
    <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/[0.03] dark:border-cyan-300/10 dark:bg-[#09152c]/95 dark:shadow-[0_20px_60px_rgba(0,0,0,0.18)] dark:ring-white/5">
      ...
      {watchPatterns.length > 0 ? (
        <div className="mt-5 rounded-2xl border border-sky-100 bg-slate-50 p-4 dark:border-cyan-300/10 dark:bg-[#030816]/70">
...
function PatternGroup({ title, patterns }: { title: string; patterns: PriceBehaviorAnalysis["candlestickPatterns"] }) {
  return (
    <div className="rounded-2xl border border-sky-100 bg-slate-50 p-4 dark:border-cyan-300/10 dark:bg-[#030816]/70">
      <h3 className="text-sm font-semibold text-slate-950 dark:text-white">{title}</h3>
